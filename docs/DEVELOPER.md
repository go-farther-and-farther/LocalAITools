...
LocalAITools 是一套**本地优先的 AI 工具集**，通过 Gradio Web UI 调用 OpenAI 兼容 API（llama.cpp / LM Studio / Ollama / 云端 API），提供图片分析、文本处理、知识库问答、LLM 压测等功能。
...
```json
{
  "providers": {
    "list": [
      {"name": "默认", "type": "openai_compatible", "base_url": "http://localhost:1234/v1", "api_key": "lm-studio"},
      {"name": "Qwen3.6-35B", "type": "llama_server", "base_url": "http://127.0.0.1:1234/v1", "api_key": "local",
       "llama_server_path": "D:\\llama\\llama-server.exe", "model_path": "C:\\models\\Qwen.gguf",
       "mmproj_path": "", "ctx_size": 64000, "port": 1234, "expert_kv": 8, "reasoning": false}
    ],
    "active": "默认"
  },
...
### 3.4 llama.cpp 服务管理（services/llama_server.py）

```
用户添加 llama_server 类型供应商
  → 填写安装目录 + 扫描选择模型
  → 保存到 state.json

用户点击 ▶️ 启动服务
  → find_server_executable(root_dir) 自动查找 llama-server.exe
  → scan_gguf_models(root_dir) 扫描 .gguf 模型
  → subprocess.Popen 启动 llama-server.exe
  → 后台线程轮询 /health 等待就绪
  → 自动检测 mmproj（视觉模型）
```

**自动检索：**
- `find_server_executable(root_dir)` — 在安装目录下搜索 llama-server.exe（直接匹配 + 3 层递归）
- `scan_gguf_models(root_dir)` — 扫描安装目录、models/ 子目录、~/.lmstudio/models/
- mmproj 自动检测 — 模型同目录下查找 mmproj*.gguf

**供应商类型系统：**
- `openai_compatible` — 连接外部 API（LM Studio、Ollama、云端等）
- `llama_server` — 管理本地 llama-server.exe 进程
- 旧格式供应商自动补全 `type: "openai_compatible"`（`_normalize_provider()`）
...
使用 `subprocess.Popen` 启动外部 llama-server.exe，后台线程轮询 `/health` 端点确认就绪。支持 terminate + kill 双重停止机制。
...
# 可选
# llama.cpp 通过供应商系统管理，无需 Python 包依赖
```
...
| V3.4 | 聊天页卡死修复（tab.select 阻塞+JS 优化）、图片工具布局重构（评分拆子标签/重命名内联分类）、子文件夹支持（重命名+分类 1-2 层）、启动脚本中文化（安装.bat/启动.bat） |
| V3.5 | 集成 llama.cpp（供应商类型系统、自动检索安装目录、扫描模型、mmproj 自动检测）、移除 llama-cpp-python 内置本地模型 |

---

*最后更新：2026-05-15 · V3.5*
