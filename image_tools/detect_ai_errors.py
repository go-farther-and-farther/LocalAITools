...
3. 构图与取景：构图是否合理，主体位置、裁剪、背景处理是否恰当，有无杂物干扰。
...
def detect_image_quality(image_path: Path, mode: str = "ai") -> Tuple[Optional[float], Optional[str], str]:
    prompt = _get_prompt(mode)
...
def process_and_classify(target_dir: str, mode: str = "ai"):
    ...
    with ThreadPoolExecutor(max_workers=config.DEFAULT_WORKERS) as executor:
        future_to_path = {executor.submit(detect_image_quality, img, mode): img for img in image_files}
